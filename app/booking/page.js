...
const Header = ({ onSearchUpdate, searchParams, onSearchParamsChange }) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // 4. MAPPING DATA (Quan trọng: Đổi tên trường để khớp với RoomListing)
      const formattedRooms = rawRoomsData.map((room) => ({
        ...
        MaPhong: room.MaPhong, // Giữ lại MaPhong gốc để booking
      }));
      ...
      alert(
        `✅ Found ${formattedRooms.length} available rooms at ${selectedHotel.TenKS}`
      );
    } catch (error) {
        ...
        >
          ...
          {showGuestPopup && (
            <div
              ...
              onClick={(e) => e.stopPropagation()}
            >
              {/* Controls for Rooms/Guests (Giữ nguyên logic cũ, chỉ style lại chút nếu cần) */}
